...
expr_site <- tryp$nci60_expr
expr_prot <- nci$ibaq
annot_site <- tryp$annot
annot_prot <- nci$annot
...
## plot
tiff("../Manuscript/supplementFigures/supp_site_prot_cor_nci60.tiff", 
...
gn2 <- f1(unique(annot_prot[fres$prot[fres$r <= 0.5], "Gene name"]))
gn3 <- f1(unique(annot_prot[fres$prot[fres$r > 0.5], "Gene name"]))
