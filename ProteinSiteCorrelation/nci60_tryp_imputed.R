...
expr_site <- tryp$fun_prepElnet(tryp, panel = "NCI60")$imputed
expr_prot <- nci$fun_prepElnet(nci, celllines = colnames(expr_site))$imputed
annot_site <- tryp$annot[rownames(expr_site), ]
annot_prot <- nci$annot[rownames(expr_prot), ]
...
## plot
tiff("../Manuscript/supplementFigures/supp_site_prot_cor_nci60_imputed.tiff", 
